    ...
    try:
        ...
        for days in test_days:
            try:
                ...
                results.append({"days": days, "clause": clause, "correct_format": is_correct})

                logger.info(
                    f"Date range test for {days} days: {'PASSED ✅' if is_correct else 'FAILED ❌'}"
                )
            except Exception as e:
...
def test_optimizer():
    ...
    try:
        ...
        mock_campaigns = [
            {"id": "123456789", "name": "Test Campaign 1", "status": "ENABLED", "days": 30}
        ]
        ...
        parsed_suggestions = optimizer.parse_suggestions(
            sample_suggestions_text, mock_campaigns, mock_keywords
        )
...
def test_chat_interface():
    ...
    try:
        ...
        for message, expected_command in commands:
            ...
            logger.info(
                f"Command detection '{message}' -> '{detected}': {'PASSED ✅' if result else 'FAILED ❌'}"
            )
        ...
        param_tests = [
            ...
            (
                "schedule daily analysis at 3:30pm",
                "schedule",
                {"hour": 15, "minute": 30, "frequency": "daily"},
            ),
        ]
        ...
        for message, command, expected_params in param_tests:
            ...
            logger.info(
                f"Parameter parsing '{message}': {'PASSED ✅' if all_correct else 'FAILED ❌'}"
            )
