    ...
    try:
        ...
        payload = {
            "contents": [
                {
                    "parts": [
                        {"text": "Write a single sentence about Google Ads."}
                    ]
                }
            ],
            "generationConfig": {
        ...
        try:
            ...
            for model_name in ['gemini-pro', 'gemini-1.0-pro', 'gemini-1.5-pro']:
                try:
                    ...
                    sdk_response = model.generate_content("Write a single sentence about Google Ads.")
            ...
            logger.info("This is expected if you're having SDK issues. Direct API approach will be used instead.")
...
def test_ads_api():
    ...
    try:
        ...
        if keyword_count > 0:
            ...
            logger.info(f"  Campaign: {first_kw['campaign_name']} (Status: {first_kw['campaign_status']})")
            logger.info(f"  Ad Group: {first_kw['ad_group_name']} (Status: {first_kw['ad_group_status']})")
            logger.info(f"  Keyword Status: {first_kw['status']}")
            logger.info(f"  Metrics: {first_kw['clicks']} clicks, {first_kw['impressions']} impressions")
