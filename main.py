    ...
    for i, campaign in enumerate(campaigns):
        ...
        try:
            ...
            if len(keywords) > MAX_KEYWORDS_PER_CALL:
                print(
                    f"  Keyword count ({len(keywords)}) exceeds limit ({MAX_KEYWORDS_PER_CALL}). Processing keywords in batches..."
                )
                ...
                for j in range(num_batches):
                    ...
                    print(
                        f"    Processing keyword batch {j + 1}/{num_batches} ({len(keyword_batch)} keywords) for campaign '{campaign_name}'..."
                    )
                    try:
                        # Get suggestions for the current keyword batch
                        batch_suggestions = optimizer.get_optimization_suggestions(
                            [campaign], keywords=keyword_batch
                        )
                        if isinstance(batch_suggestions, list):
                            # Filter out parser errors before extending
                            valid_batch_suggestions = [
                                s for s in batch_suggestions if s.get("action_type") != "ERROR"
                            ]
                            if valid_batch_suggestions:
                                campaign_batch_suggestions.extend(valid_batch_suggestions)
                                print(
                                    f"      Received {len(valid_batch_suggestions)} valid suggestions for batch {j + 1}."
                                )
                            elif batch_suggestions:  # Log if only errors were returned by parser
                                logging.warning(
                                    f"      Optimizer/parser returned only errors for batch {j + 1}: {batch_suggestions}"
                                )
                        else:
                            logging.warning(
                                f"      Received unexpected suggestion format for batch {j + 1}: {batch_suggestions}"
                            )
                    except Exception as batch_e:
                        logging.error(
                            f"      ERROR processing keyword batch {j + 1} for campaign '{campaign_name}': {str(batch_e)}"
                        )
                ...
                print(
                    f"  Finished processing {num_batches} keyword batches for campaign '{campaign_name}'. Total suggestions for campaign: {len(campaign_batch_suggestions)}."
                )

            elif len(keywords) > 0:
                # Process all keywords at once if count is within limit and > 0
                print(
                    f"  Getting optimization suggestions for campaign {campaign_id} (all {len(keywords)} keywords at once)..."
                )
                campaign_suggestions = optimizer.get_optimization_suggestions(
                    [campaign], keywords=keywords
                )
                if isinstance(campaign_suggestions, list):
                    valid_suggestions = [
                        s for s in campaign_suggestions if s.get("action_type") != "ERROR"
                    ]
                    if valid_suggestions:
                        print(
                            f"  Received {len(valid_suggestions)} valid suggestions for campaign '{campaign_name}'."
                        )
                        all_suggestions.extend(valid_suggestions)
                    elif campaign_suggestions:  # Log if only errors were returned by parser
                        logging.warning(
                            f"  Optimizer/parser returned only errors for campaign '{campaign_name}': {campaign_suggestions}"
                        )
                else:
                    logging.warning(
                        f"  Received unexpected suggestion format for campaign '{campaign_name}': {campaign_suggestions}"
                    )
            else:
                # No keywords to process for this campaign
                print(
                    f"  No keywords found or processed for campaign '{campaign_name}'. Skipping suggestions."
                )
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=90, help="Number of days to look back for campaign data"
    )
    parser.add_argument(
        "--schedule",
        action="store_true",
        help="Run the scheduler instead of a one-time optimization",
    )
    parser.add_argument("--hour", type=int, default=9, help="Hour to run the scheduled task (0-23)")
    parser.add_argument(
        "--minute", type=int, default=0, help="Minute to run the scheduled task (0-59)"
    )
