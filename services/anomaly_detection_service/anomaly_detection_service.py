...
class AnomalyDetectionService(BaseService):
    ...
    def detect_anomalies(
        self, data: pd.DataFrame, entity_type: str = "campaign", methods: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if len(data) < self.config["min_data_points"]:
            self.logger.warning(
                f"Insufficient data for anomaly detection. Got {len(data)} points, need {self.config['min_data_points']}"
            )
            return []
        ...
        if (
            self.config["seasonality_adjust"] and len(data) >= 14
        ):  # Need at least 2 weeks for weekday patterns
        ...
        sorted_anomalies = sorted(
            unique_anomalies, key=lambda x: (-x.get("severity_score", 0), x.get("date"))
        )
    ...
    def detect_statistical_anomalies(
        self, data: pd.DataFrame, entity_type: str = "campaign"
    ) -> List[Dict[str, Any]]:
        """
                ...
                if is_zscore_anomaly or is_iqr_anomaly:
                    ...
                    anomaly = {
                        ...
                        "entity_id": (
                            ...
                            else data.get("entity_id", "unknown")
                        ),
                        "entity_name": (
                            data.get("name", "unknown")
                            if "name" in data.columns
                            else data.get("entity_name", "unknown")
                        ),
                        "date": (
                        ...
                        "direction": direction,
                        "percent_change": (
                            float((value - mean) / mean * 100) if mean != 0 else float("inf")
                        ),
                ...
                if predictions[idx] == -1:  # -1 indicates anomaly
                    ...
                    anomaly = {
                        ...
                        "entity_id": (
                            row.get("id", "unknown")
                            if "id" in row.index
                            else row.get("entity_id", "unknown")
                        ),
                        "entity_name": (
                            row.get("name", "unknown")
                            if "name" in row.index
                            else row.get("entity_name", "unknown")
                        ),
                        "date": (
                            date.strftime("%Y-%m-%d") if hasattr(date, "strftime") else str(date)
                        ),
                        ...
                        "direction": direction,
                        "percent_change": (
                            float((anomalous_value - expected_value) / expected_value * 100)
                            if expected_value != 0
                            else float("inf")
                        ),
            ...
            for date, value in values.items():
                ...
                if is_anomaly:
                    ...
                    anomaly = {
                        ...
                        "direction": direction,
                        "percent_change": (
                            float((value - avg_value) / avg_value * 100)
                            if avg_value != 0
                            else float("inf")
                        ),
    ...
    def get_anomalies(
        ...
        if start_date:
            ...
            filtered = [
                a
                for a in filtered
                if datetime.strptime(a.get("date", "1970-01-01"), "%Y-%m-%d").date() >= start
            ]
        ...
        if end_date:
            ...
            filtered = [
                a
                for a in filtered
                if datetime.strptime(a.get("date", "2099-12-31"), "%Y-%m-%d").date() <= end
            ]
        ...
        sorted_anomalies = sorted(
            filtered, key=lambda x: (-x.get("severity_score", 0), x.get("date"))
        )
    ...
    def generate_alerts(
        self, anomalies: Optional[List[Dict[str, Any]]] = None, min_severity: float = 7.0
    ) -> List[Dict[str, Any]]:
        """
        ...
        # Filter by severity
        high_severity = [
            a for a in anomalies_to_process if a.get("severity_score", 0) >= min_severity
        ]
            ...
            if "day_of_week" in adjusted_data.columns:
                ...
                for day, factor in factors.items():
                    if factor > 0:
                        ...
                        adjusted_data.loc[mask, f"{metric}_adjusted"] = (
                            adjusted_data.loc[mask, metric] / factor
                        )
    ...
    def _deduplicate_anomalies(self, anomalies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for anomaly in anomalies:
            ...
            if key not in grouped or anomaly.get("severity_score", 0) > grouped[key].get(
                "severity_score", 0
            ):
                grouped[key] = anomaly
