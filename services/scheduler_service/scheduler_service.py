...
class SchedulerService(BaseService):
        ...
        if schedule_type == "one_time":
            ...
            try:
                ...
                if seconds_until > 0:
                    ...
                    self.scheduler.every(seconds_until).seconds.do(self._execute_task, task).tag(task_id)
                    task["next_run"] = schedule_datetime.isoformat()
                else:
                    self.logger.warning(f"Task {task_id} scheduled in the past, will not be executed")
                    task["status"] = "expired"
            except ValueError:
            ...
            try:
                ...
                job = self.scheduler.every().day.at(schedule_time).do(self._execute_task, task).tag(task_id)
            ...
            try:
                ...
                if day.lower() in day_methods:
                    job = day_methods[day.lower()].at(f"{hour:02d}:{minute:02d}").do(self._execute_task, task).tag(task_id)
            ...
            try:
                ...
                job = self.scheduler.every().day.at(time_part).do(monthly_job_wrapper, task).tag(task_id)
    ...
    def run(self, **kwargs):
        ...
        for task in self.tasks["tasks"]:
            if task["enabled"] and task["status"] == "pending" and task["schedule_type"] == "one_time":
                try:
                    next_run = datetime.fromisoformat(task["next_run"]) if task.get("next_run") else None
