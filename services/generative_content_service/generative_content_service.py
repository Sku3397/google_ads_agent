...
class GenerativeContentService(BaseService):
    ...
    def generate_headlines(self, 
                          ...
                          num_headlines: int = 5, 
                          tone: str = "professional") -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.optimizer:
                ...
                if not descriptions:
                    ...
                    descriptions = self._generate_fallback_descriptions(keywords, product_info, num_descriptions)
            else:
                # If no optimizer, use fallback method
                descriptions = self._generate_fallback_descriptions(keywords, product_info, num_descriptions)
    ...
    def analyze_ad_performance(self, 
        ...
        """
        start_time = datetime.now()
        self.logger.info(f"Analyzing ad performance {f'for campaign {campaign_id}' if campaign_id else ''}")
    ...
    def suggest_responsive_search_ad(self, 
                                    keywords: List[str], 
                                    product_info: Dict[str, Any],
                                    competitor_ads: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(f"Generated RSA suggestion with {len(headlines)} headlines and {len(descriptions)} descriptions")
            self._track_execution(start_time, success=True)
    ...
    def _generate_fallback_headlines(self, keywords: List[str], product_name: str, count: int) -> List[Dict[str, Any]]:
        ...
        headlines = []
        benefits = ["Great Value", "High Quality", "Fast Service", "Professional Results", "24/7 Support"]
        ...
        for i in range(min(count, len(templates))):
            ...
            headlines.append({
                "headline": headline,
                "rationale": "Fallback template-based headline"
            })
            
        return headlines
    
    def _generate_fallback_descriptions(self, keywords: List[str], product_info: Dict[str, Any], count: int) -> List[Dict[str, Any]]:
        ...
        features = product_info.get("features", ["high quality", "expert design", "premium service"])
        benefits = product_info.get("benefits", ["great results", "significant savings", "peace of mind"])
        offers = ["free shipping", "10% off", "a free consultation", "next-day delivery", "24/7 support"]
        ...
        for i in range(min(count, len(templates))):
            ...
            descriptions.append({
                ...
                "rationale": "Fallback template-based description"
            })
    ...
    def _generate_display_path(self, keywords: List[str], product_info: Dict[str, Any], is_second: bool = False) -> str:
    ...
    def run(self, **kwargs):
        ...
        if action == "generate_headlines":
            ...
            return self.analyze_ad_performance(
                kwargs.get("ad_performance_data", []),
