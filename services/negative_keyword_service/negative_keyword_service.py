...
class NegativeKeywordService(BaseService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.config.get('negative_keyword', None):
            ...
            self.max_negative_suggestions = neg_kw_config.get('max_negative_suggestions', self.max_negative_suggestions)
            self.min_cost_without_conversion = neg_kw_config.get('min_cost_without_conversion', self.min_cost_without_conversion)
        ...
        self.compiled_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.irrelevant_patterns]
        
        self.logger.info(f"NegativeKeywordService initialized with thresholds: "
    ...
    def identify_negative_keywords(self, days: int = 30, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            campaign_level_negatives = self._identify_campaign_level_negatives(new_negatives, campaign_id)
            ...
            result = {
                ...
                "campaign_level_negatives": campaign_level_negatives[:self.max_negative_suggestions],
                "ad_group_level_negatives": ad_group_level_negatives[:self.max_negative_suggestions]
            }
            ...
            self.logger.info(f"Negative keyword identification completed successfully with {len(new_negatives)} suggestions")
    ...
    def _get_search_terms_report(self, days: int, campaign_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def _identify_zero_conversion_terms(self, search_terms: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        self.logger.info(f"Identified {len(zero_conversion_terms)} search terms with zero conversions")
        return zero_conversion_terms
    
    def _identify_pattern_match_terms(self, search_terms: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        self.logger.info(f"Identified {len(pattern_match_terms)} search terms matching irrelevant patterns")
        return pattern_match_terms
    
    def _combine_negative_candidates(self, 
                                    low_ctr_terms: List[Dict[str, Any]],
                                    zero_conversion_terms: List[Dict[str, Any]],
                                    pattern_match_terms: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for term_list in [low_ctr_terms, zero_conversion_terms, pattern_match_terms]:
            for term in term_list:
                ...
                if query in combined:
                    ...
                    combined[query]["confidence"] = min(0.95, combined[query].get("confidence", 0.7) + 0.1)
                else:
    ...
    def _filter_out_existing_negatives(self, 
        ...
        self.logger.info(f"Filtered {len(candidates) - len(new_negatives)} existing negative keywords")
        return new_negatives
    
    def _identify_account_level_negatives(self, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _identify_campaign_level_negatives(self, 
                                         candidates: List[Dict[str, Any]], 
    ...
    def _identify_ad_group_level_negatives(self, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _add_negative_keyword(self, 
        ...
        """
        self.logger.info(f"Adding negative keyword '{query}' at {level} level with {match_type} match type")
        ...
        if level == "account":
            ...
            if not campaign_id:
                return {"success": False, "query": query, "level": level, "error": "No campaign ID provided"}
            
            return {"success": True, "query": query, "level": level, "campaign_id": campaign_id, "match_type": match_type}
        elif level == "ad_group":
            # Add to ad-group-level negatives
            ad_group_id = recommendation.get("ad_group_id", "")
            if not ad_group_id:
                return {"success": False, "query": query, "level": level, "error": "No ad group ID provided"}
            
            return {"success": True, "query": query, "level": level, "ad_group_id": ad_group_id, "match_type": match_type}
        else:
    ...
    def analyze_negative_keyword_impact(self, days_before: int = 30, days_after: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.info(f"Analyzing negative keyword impact ({days_before} days before, {days_after} days after)")
            ...
            self.logger.info(f"Completed impact analysis for {len(recent_negatives)} negative keywords")
            self._track_execution(start_time, True)
    ...
    def _analyze_single_negative_impact(self, 
