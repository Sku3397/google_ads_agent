...
class KeywordService(BaseService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.config.get('keyword', None):
            ...
            self.max_keyword_suggestions = keyword_config.get('max_keyword_suggestions', self.max_keyword_suggestions)
            self.min_impression_share = keyword_config.get('min_impression_share', self.min_impression_share)
            self.min_relevance_score = keyword_config.get('min_relevance_score', self.min_relevance_score)
        
        self.logger.info(f"KeywordService initialized with settings: "
    ...
    def discover_new_keywords(self, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Starting keyword discovery for campaign_id={campaign_id or 'all campaigns'}")
            ...
            result = {
                ...
                "search_terms_analysis": {
                    ...
                    "high_performing_terms": [t for t in search_terms if t.get("ctr", 0) > 0.1][:10],
                    "converting_terms": [t for t in search_terms if t.get("conversions", 0) > 0][:10]
                }
            }
            ...
            self.logger.info(f"Keyword discovery completed successfully with {len(all_suggestions)} suggestions")
    ...
    def _get_current_keywords(self, campaign_id: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        """
        try:
            self.logger.info(f"Fetching current keywords for campaign_id={campaign_id or 'all campaigns'}")
    ...
    def _analyze_search_terms_report(self, campaign_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def _get_keyword_planner_suggestions(self, 
    ...
    def _get_ai_keyword_suggestions(self,
                                  current_keywords: List[Dict[str, Any]],
    ...
    def _combine_keyword_suggestions(self,
                                   keyword_planner_suggestions: List[Dict[str, Any]],
                                   ai_suggestions: List[Dict[str, Any]],
                                   current_keywords: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def _group_keywords_into_ad_groups(self, keyword_suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    def analyze_keyword_performance(self, days: int = 30, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            df["ctr"] = df.apply(lambda row: row.get("clicks", 0) / row.get("impressions", 1) if row.get("impressions", 0) > 0 else 0, axis=1)
            df["conversion_rate"] = df.apply(lambda row: row.get("conversions", 0) / row.get("clicks", 1) if row.get("clicks", 0) > 0 else 0, axis=1)
            df["cost_per_conversion"] = df.apply(lambda row: row.get("cost", 0) / row.get("conversions", 1) if row.get("conversions", 0) > 0 else 0, axis=1)
            df["impression_share"] = df["search_impression_share"].apply(lambda x: float(x) if isinstance(x, (int, float)) else 0)
            ...
            recommendations = self._generate_keyword_recommendations(
                high_performing, low_ctr, low_quality_score, expensive_keywords, impression_opportunity
            )
            ...
            analysis = {
                ...
                "average_ctr": df["clicks"].sum() / df["impressions"].sum() if df["impressions"].sum() > 0 else 0,
                "average_conversion_rate": df["conversions"].sum() / df["clicks"].sum() if df["clicks"].sum() > 0 else 0,
                "performance_groups": {
    ...
    def _generate_keyword_recommendations(self,
                                        ...
                                        impression_opportunity: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for keyword in low_ctr[:10]:  # Limit to top 10
            recommendations.append({
                ...
                "keyword_text": keyword.get("keyword_text", ""),
                "current_bid": keyword.get("current_bid", 0),
                "recommended_bid": keyword.get("current_bid", 0) * 0.7 if keyword.get("conversions", 0) > 0 else 0,
