...
class DataVisualizationService(BaseService):
    ...
    def create_campaign_performance_chart(self, 
        ...
        try:
            ...
            pivot_df = df.pivot_table(index='date', columns='campaign_name', values=metric, aggfunc='sum')
            ...
            for i, campaign in enumerate(pivot_df.columns):
                ...
                pivot_df[campaign].plot(ax=ax, label=campaign, linewidth=2, color=color, marker='o', markersize=4)
    ...
    def create_keyword_performance_chart(self, 
        ...
        try:
            ...
            for i, bar in enumerate(bars):
                ...
                ax.text(label_x, bar.get_y() + bar.get_height()/2, value_text, 
    ...
    def create_performance_summary_dashboard(self, 
        ...
        try:
            ...
            if 'date' in df.columns and 'clicks' in df.columns and 'impressions' in df.columns:
                daily_metrics = df.groupby('date').agg({'clicks': 'sum', 'impressions': 'sum'}).reset_index()
                
                # Plot clicks
                color1 = self.colors[0]
                lns1 = ax1.plot(daily_metrics['date'], daily_metrics['clicks'], 
                               color=color1, marker='o', label='Clicks')
                ...
                lns2 = ax1b.plot(daily_metrics['date'], daily_metrics['impressions'], 
                                color=color2, marker='s', label='Impressions')
            ...
            if 'campaign_name' in df.columns and 'clicks' in df.columns:
                ...
                campaign_perf['cvr'] = campaign_perf['conversions'] / campaign_perf['clicks'].replace(0, 1)
                ...
                bars = ax2.bar(top_campaigns['campaign_name'], top_campaigns['clicks'], color=colors)
            ...
            if 'date' in df.columns and 'cost' in df.columns and 'conversions' in df.columns:
                daily_metrics = df.groupby('date').agg({'cost': 'sum', 'conversions': 'sum'}).reset_index()
                
                # Plot cost
                color1 = self.colors[2]
                lns1 = ax3.plot(daily_metrics['date'], daily_metrics['cost'], 
                               color=color1, marker='o', label='Cost')
                ...
                lns2 = ax3b.plot(daily_metrics['date'], daily_metrics['conversions'], 
                                color=color2, marker='s', label='Conversions')
            ...
            metrics_summary = {
                ...
                'Avg. CTR': df['clicks'].sum() / df['impressions'].sum() if 'clicks' in df.columns and 'impressions' in df.columns and df['impressions'].sum() > 0 else 0,
                'Avg. CPC': df['cost'].sum() / df['clicks'].sum() if 'cost' in df.columns and 'clicks' in df.columns and df['clicks'].sum() > 0 else 0,
                'Conv. Rate': df['conversions'].sum() / df['clicks'].sum() if 'conversions' in df.columns and 'clicks' in df.columns and df['clicks'].sum() > 0 else 0,
                'Cost/Conv.': df['cost'].sum() / df['conversions'].sum() if 'cost' in df.columns and 'conversions' in df.columns and df['conversions'].sum() > 0 else 0
            }
            ...
            plt.suptitle(f"Google Ads Performance Dashboard - Last {days} Days", fontsize=18, y=0.98)
