...
class BidService(BaseService):
    ...
    def __init__(
        ...
        if self.config.get("bid", None):
            ...
            self.max_bid_increase_pct = bid_config.get(
                "max_bid_increase_pct", self.max_bid_increase_pct
            )
            self.max_bid_decrease_pct = bid_config.get(
                "max_bid_decrease_pct", self.max_bid_decrease_pct
            )
            self.min_conversions = bid_config.get("min_conversions", self.min_conversions)
            self.update_frequency_days = bid_config.get(
        ...
        self.logger.info(
            f"BidService initialized with settings: "
    ...
    def _get_keyword_performance_data(
        ...
        try:
            ...
            filtered_keywords = [
                kw for kw in keywords if kw.get("impressions", 0) >= self.min_data_points
            ]

            self.logger.info(
                f"Fetched {len(keywords)} keywords, {len(filtered_keywords)} with sufficient data"
            )
    ...
    def _optimize_bids_performance_based(
    ...
    def _optimize_bids_target_cpa(self, keywords: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if not self.target_cpa:
            converting_keywords = [
                kw for kw in keywords if kw.get("conversions", 0) >= self.min_conversions
            ]
            ...
            if conversions >= self.min_conversions:
                ...
                # If current CPA is lower than target, can increase bid
                    ...
                    adjustment_factor = min(
                        1.2, self.target_cpa / current_cpa
                    )  # Cap at 20% increase
                    recommended_bid = current_bid * adjustment_factor
    ...
    def _optimize_bids_target_roas(self, keywords: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        if not self.target_roas:
            ...
            if not self.target_roas:
                self.logger.warning(
                    "Could not calculate target ROAS, insufficient conversion value data"
                )
                return []
    ...
    def apply_bid_recommendations(self, recommendations: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Applied {success_count} bid recommendations, {failure_count} failures"
            )
    ...
    def optimize_campaign_budgets(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Campaign budget optimization completed with {len(final_recommendations)} recommendations"
            )
            self._track_execution(start_time, True)
    ...
    def _get_campaign_performance_data(self, days: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Fetched {len(campaigns)} campaigns, {len(active_campaigns)} are active"
            )
    ...
    def _apply_budget_safety_checks(
        self, recommendations: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            for criterion_id, new_bid in adjustments.items():
                ...
                criterion.resource_name = (
                    f"customers/{self.customer_id}/adGroupCriteria/{criterion_id}"
                )
                criterion.cpc_bid_micros = bid_micros
                operation.update_mask.paths.append("cpc_bid_micros")

                operations.append(operation)
                results[criterion_id] = {"new_bid": new_bid, "status": "pending"}
            ...
            if operations:
                ...
                for i, result in enumerate(response.results):
                    ...
                    results[criterion_id][
                        "message"
                    ] = f"Bid updated to {results[criterion_id]['new_bid']}"
