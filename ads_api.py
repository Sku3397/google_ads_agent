...
class GoogleAdsAPI:
    ...
    def __init__(self, config: Dict[str, str]) -> None:
        ...
        Args:
            config: Configuration dictionary with Google Ads credentials
        """
    ...
    def _get_date_range_clause(self, days_ago: int) -> str:
        """
        Generate a date range clause for Google Ads API queries.

        Args:
            days_ago: Number of days to look back (1-365)

        Returns:
            Date range clause for Google Ads API
        """
        ...
        logger.info(f"Using date range from {start_str} to {end_str} ({days_ago} days)")

        # Return the date range clause in the format Google Ads API expects
        return f"segments.date BETWEEN '{start_str}' AND '{end_str}'"

    def get_campaign_performance(self, days_ago: int = 30) -> List[Dict[str, Any]]:
        """
        Fetch minimal campaign performance data from Google Ads.

        Args:
            days_ago: Number of days to look back for data (1-365)

        Returns:
            List of campaign data dictionaries (minimal fields)
        """
        # Validate input
        ...
        try:
            ...
            logger.info(f"Fetching campaign data (ENABLED only) for the last {days_ago} days")
            ...
            for row in response:
                campaign = {
                    ...
                    "impressions": (
                        row.metrics.impressions if hasattr(row.metrics, "impressions") else 0
                    ),
                    "clicks": row.metrics.clicks if hasattr(row.metrics, "clicks") else 0,
                    "conversions": (
                        row.metrics.conversions if hasattr(row.metrics, "conversions") else 0
                    ),
                    "cost": (
    ...
    def get_keyword_performance(
        self, days_ago: int = 30, campaign_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        Returns:
            List of keyword data dictionaries (reduced fields)
        """
        ...
        logger.info(f"Using keyword filter: {where_clause}")
        ...
        for metric in removed_metrics:
            if metric in query:
                logger.warning(
                    f"Found removed metric {metric} in query. Please update the query definition."
                )
        ...
        try:
            ...
            logger.info(
                f"Fetching reduced keyword data for the last {days_ago} days{' for campaign ' + campaign_id if campaign_id else ''}"
            )
            response = ga_service.search(customer_id=self.customer_id, query=query)
                ...
                try:
                    ...
                    keyword = {
                        ...
                        "system_serving_status": (
                            row.ad_group_criterion.system_serving_status.name
                            if hasattr(row.ad_group_criterion, "system_serving_status")
                            else None
                        ),
                        "quality_score": (
                            row.ad_group_criterion.quality_info.quality_score
                            if hasattr(row.ad_group_criterion, "quality_info")
                            and hasattr(row.ad_group_criterion.quality_info, "quality_score")
                            else 0
                        ),
                        "current_bid": (
                            row.ad_group_criterion.effective_cpc_bid_micros / 1_000_000
                            if hasattr(row.ad_group_criterion, "effective_cpc_bid_micros")
                            and row.ad_group_criterion.effective_cpc_bid_micros
                            else 0
                        ),
                        "criterion_id": (
                            row.ad_group_criterion.criterion_id
                            if hasattr(row.ad_group_criterion, "criterion_id")
                            else None
                        ),
                        "resource_name": (
                            row.ad_group_criterion.resource_name
                            if hasattr(row.ad_group_criterion, "resource_name")
                            else None
                        ),
                        "is_negative": (
                            row.ad_group_criterion.negative
                            if hasattr(row.ad_group_criterion, "negative")
                            else False
                        ),
                        "clicks": row.metrics.clicks if hasattr(row.metrics, "clicks") else 0,
                        "impressions": (
                        ...
                        "cost": (
                            row.metrics.cost_micros / 1_000_000 if row.metrics.cost_micros else 0
                        ),
                        "top_impression_pct": (
                            row.metrics.top_impression_percentage
                            if hasattr(row.metrics, "top_impression_percentage")
                            else None
                        ),
                        ...
                        "search_top_impression_share": (
                            ...
                            if hasattr(row.metrics, "search_top_impression_share")
                            else None
                        ),
                        "days": days_ago,
                    }
                    ...
                    logger.warning(f"Attribute error processing keyword row: {ae}. Row data: {row}")
                    continue  # Skip this keyword row if essential attributes are missing

            logger.info(f"Found {len(keywords)} keywords with data in the specified time period.")
            return keywords
            ...
            logger.exception(error_message)  # Log traceback for unexpected errors
            raise Exception(error_message)

    def apply_optimization(
        self, optimization_type: str, entity_type: str, entity_id: str, changes: Dict[str, Any]
    ) -> Tuple[bool, str]:
        """
        Apply an optimization to Google Ads.

        Args:
            optimization_type: Type of optimization (e.g., 'bid_adjustment', 'status_change')
            entity_type: Type of entity to optimize (e.g., 'keyword', 'campaign', 'ad_group')
            entity_id: ID of the entity to optimize
        ...
        try:
            if entity_type == "keyword" and optimization_type == "bid_adjustment":
                ...
                return self._add_keyword(
                    ...
                    changes.get("bid_micros", 1_000_000),  # Default to $1.00
                )
            else:
    ...
    def _adjust_keyword_bid(self, keyword_criterion_id: str, bid_micros: int) -> Tuple[bool, str]:
        """
        Adjust the bid for a keyword.

        Args:
            keyword_criterion_id: ID of the ad group criterion (keyword)
            bid_micros: New bid in micros
        ...
        try:
            ...
            logger.warning(f"Unable to check if keyword is negative: {str(ex)}")

        except Exception as e:
            logger.warning(f"Error checking if keyword is negative: {str(e)}")
        ...
        try:
            ...
            return (
                True,
                f"Successfully updated bid for keyword {criterion_id} to ${bid_micros / 1_000_000:.2f}",
            )
    ...
    def _update_keyword_status(self, keyword_criterion_id: str, status: str) -> Tuple[bool, str]:
        """
    ...
    def _update_campaign_status(self, campaign_id: str, status: str) -> Tuple[bool, str]:
        """
    ...
    def _update_campaign_budget(self, campaign_id: str, budget_micros: int) -> Tuple[bool, str]:
        """
        Update the budget of a campaign.

        Args:
            campaign_id: ID of the campaign
            budget_micros: New budget in micros
        ...
        try:
            ...
            return True, f"Successfully updated campaign budget to ${budget_micros / 1_000_000:.2f}"
    ...
    def _add_keyword(
        ...
        Args:
            ...
            keyword_text: Text of the keyword to add
            match_type: Match type (EXACT, PHRASE, BROAD)
            bid_micros: Bid amount in micros
        ...
        try:
            ...
            if not ad_group_id:
                ...
                logger.info(
                    f"Using ad group '{ad_group.ad_group.name}' (ID: {ad_group_id}) for new keyword"
                )
            ...
            return (
                True,
                f"Successfully added keyword '{keyword_text}' with {match_type} match type and ${bid_micros / 1_000_000:.2f} bid to ad group {ad_group_id}",
            )
