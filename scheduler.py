...
class AdsScheduler:
    ...
    def add_task(
        ...
        if self.logger:
            self.logger.info(
                f"Task {name} (ID: {task_id}) scheduled for {schedule_type} at {hour:02d}:{minute:02d}"
    ...
    def _schedule_task(self, task):
        ...
        def task_wrapper():
            try:
                ...
                if self.logger:
                    self.logger.error(
                        f"Task failed: {task.name} (ID: {task.task_id}). Error: {str(e)}"
                    )
        ...
        if task.schedule_type == "daily":
            ...
            if days_ahead < 0 or (
                days_ahead == 0 and now.replace(hour=task.hour, minute=task.minute) < now
            ):
                days_ahead += 7
    ...
    def schedule_weekly(
        self, function, day_of_week, hour=9, minute=0, name=None, args=None, kwargs=None
    ):
        """
    ...
    def _load_tasks(self):
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f"Found {len(serialized_tasks)} previously scheduled tasks in {self.tasks_file}"
                )
...
# Self Play Service scheduled tasks
def schedule_self_play_tasks(scheduler, self_play_service, logger=None):
    """
    Schedule Self Play Service tasks.

    Args:
        scheduler: The scheduler instance
        self_play_service: The Self Play Service instance
        logger: Optional logger instance
    """
    # Schedule weekly population initialization (Monday at 1 AM)
    scheduler.schedule_weekly(
        function=self_play_service.initialize_population,
        day_of_week="monday",
        hour=1,
        minute=0,
        name="Initialize Self Play Agent Population",
        kwargs={"campaign_id": None},  # Will use all campaigns
    )

    # Schedule weekly tournaments (Wednesday at 2 AM)
    scheduler.schedule_weekly(
        function=self_play_service.run_tournament,
        day_of_week="wednesday",
        hour=2,
        minute=0,
        name="Run Self Play Tournament",
        kwargs={"campaign_id": None},  # Will use all campaigns
    )

    # Schedule weekly strategy reports (Thursday at 3 AM)
    scheduler.schedule_weekly(
        function=self_play_service.generate_strategy_report,
        day_of_week="thursday",
        hour=3,
        minute=0,
        name="Generate Self Play Strategy Report",
        kwargs={"tournament_id": None},  # Will use the latest tournament
    )

    if logger:
        logger.info("Self Play Service tasks scheduled successfully")
