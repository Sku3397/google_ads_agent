...
def run_daily_optimization():
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            logger.info(
                f"Generated {recs['total']} recommendations, {recs['implemented']} implemented automatically"
            )
...
def run_one_time_analysis(days=30, entity_type="all", auto_implement=False):
    ...
    try:
        ...
        if auto_implement:
            implemented, pending = agent.execute_recommendations(
                recommendations, auto_implement=True
            )
            logger.info(
                f"Auto-implemented {len(implemented)} recommendations, {len(pending)} pending"
            )
        else:
            # Just summarize recommendations without implementing
            high_confidence = len(
                [r for r in recommendations if r.confidence_score >= agent.auto_implement_threshold]
            )
            logger.info(
        ...
        with open(rec_file, "w") as f:
            json.dump(
                [agent._recommendation_to_dict(r) for r in recommendations],
                f,
                indent=2,
                default=str,
            )
        ...
        # Save report
        report_file = (
            f"reports/analysis_report_{entity_type}_{datetime.now().strftime('%Y%m%d')}.json"
        )
        with open(report_file, "w") as f:
            json.dump(report, f, indent=2, default=str)
...
def main():
    ...
    analyze_parser.add_argument(
        "--entity-type",
        choices=["all", "campaigns", "keywords"],
        default="all",
    ...
    if args.command == "run-daily":
        ...
        run_one_time_analysis(
            days=args.days, entity_type=args.entity_type, auto_implement=args.auto_implement
        )
