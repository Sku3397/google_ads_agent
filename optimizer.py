...
class AdsOptimizer:
        ...
        try:
            ...
            if '404' in error_message:
                if 'models/' in error_message:
                    model_name = error_message.split('models/')[1].split(' ')[0] if 'models/' in error_message else "unknown"
                    logger.error(f"Model not found: {model_name}")
                    raise Exception(f"Model '{model_name}' not supported by the current API version. Try updating to the latest SDK version.")
                else:
                ...
                raise Exception(f"Authorization error. Please check your API key and permissions: {error_message}")
            else:
            ...
            for model_name in model_names:
                try:
                    ...
                    if response.status_code != 200:
                        ...
                        logger.error(f"Gemini API error with {model_name}: {response.status_code} - {error_message}")
                    ...
                    logger.info(f"Successfully generated content using direct API with {model_name}")
    ...
    def format_keyword_data(self, keywords: List[Dict[str, Any]], 
                           limit: int = 100, 
                           include_best: bool = True, 
                           include_worst: bool = True) -> str:
        """
        ...
        active_keywords = [kw for kw in keywords 
                          if (kw.get('status') == 'ENABLED' and 
                              kw.get('ad_group_status') == 'ENABLED' and
                              kw.get('campaign_status') == 'ENABLED')]
        
        logger.info(f"Formatting keyword data: {len(keywords)} total keywords, {len(active_keywords)} active keywords")
        
        # If no active keywords, return early with information
        if not active_keywords:
            return f"No active keywords found for analysis. Total keywords received: {len(keywords)}."
        ...
        if not keywords_with_data:
            ...
            logger.info(f"Few keywords with significant data: falling back to {len(keywords_with_data)} keywords with any impressions")
            
            # If still no data, just use active keywords
            if not keywords_with_data:
                keywords_with_data = active_keywords
                logger.info(f"No keywords with impressions. Using all {len(active_keywords)} active keywords.")
        ...
        best_keywords = sorted(keywords_with_data, 
                              key=lambda x: x.get('ctr', 0), 
                              reverse=True)[:limit//2]
        
        worst_keywords = sorted(keywords_with_data, 
                               key=lambda x: x.get('ctr', 0))[:limit//2]
        ...
        if include_best and best_keywords:
            ...
            for idx, kw in enumerate(best_keywords, 1):
                result += f"{idx}. '{kw.get('keyword_text', '')}' [{kw.get('match_type', 'Unknown')}]\n"
    ...
    def get_keyword_suggestions(self, business_info: Dict[str, Any], 
                               existing_keywords: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def get_optimization_suggestions(self, 
                                    ...
                                    business_info: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            suggestions = self._parse_json_from_response(response_text)
            logger.info(f"Successfully parsed optimization suggestions with {len(suggestions.get('keyword_recommendations', []))} keyword recommendations")
            return suggestions
        except Exception as e:
