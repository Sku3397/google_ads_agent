...
class Recommendation:
    ...
    def to_dict(self):
        return {
            ...
            "status": self.status,
            "implementation_time": (
                self.implementation_time.isoformat() if self.implementation_time else None
            ),
...
class AutonomousPPCAgent:
    ...
    def __init__(self, ads_api: GoogleAdsAPI, optimizer: AdsOptimizer, config: Dict):
        ...
        self.auto_implement_threshold = config.get(
            "auto_implement_threshold", 80
        )  # Confidence threshold for auto-implementation
        self.max_daily_budget_change_pct = config.get(
            "max_daily_budget_change_pct", 20
        )  # Max % to change budget in a day
        self.min_data_points_required = config.get(
            "min_data_points_required", 30
        )  # Min data points needed for high confidence
        self.max_keyword_bid_adjustment = config.get(
            "max_keyword_bid_adjustment", 50
        )  # Max % to adjust bids by
        self.time_periods = config.get(
            "time_periods", [7, 14, 30, 90]
        )  # Default time periods to analyze
    ...
    def analyze_and_recommend(
        self, entity_type: str = "all", days: int = 30
    ) -> List[Recommendation]:
        """
        ...
        high_confidence = len(
            [r for r in recommendations if r.confidence_score >= self.auto_implement_threshold]
        )
        logger.info(
            f"Generated {len(recommendations)} recommendations, {high_confidence} with high confidence"
        )
    ...
    def _calculate_confidence(
        self, entity_type: str, action_type: str, data_points: int, priority: str
    ) -> float:
        """
        ...
        priority_factor = {"HIGH": 1.1, "MEDIUM": 1.0, "LOW": 0.8}.get(priority, 1.0)
    ...
    def _estimate_impact(self, entity_type: str, action_type: str, priority: str) -> float:
        ...
        # Adjust based on priority
        priority_factor = {"HIGH": 1.3, "MEDIUM": 1.0, "LOW": 0.7}.get(priority, 1.0)
    ...
    def execute_recommendations(
        self, recommendations: List[Recommendation], auto_implement: bool = True
        ...
        for rec in recommendations:
            ...
            should_implement = (
                auto_implement
                and rec.confidence_score >= self.auto_implement_threshold
                and self._is_safe_to_implement(rec)
            )
    ...
    def _recommendation_to_dict(self, rec: Recommendation) -> Dict:
        ...
        return {
            ...
            "status": rec.status,
            "implementation_time": (
                rec.implementation_time.isoformat() if rec.implementation_time else None
            ),
    ...
    def track_recommendation_outcomes(self, days_to_analyze: int = 7):
        ...
        for rec in implemented_recs:
            ...
            implementation_time = (
                datetime.fromisoformat(rec["implementation_time"])
                if rec["implementation_time"]
                else None
            )
            if (
                not implementation_time
                or (datetime.now() - implementation_time).days < days_to_analyze
            ):
                continue
    ...
    def improve_decision_making(self):
        ...
        if len(actual_impacts) > 0:
            accuracy = sum(
                1 for i, p in zip(actual_impacts, predicted_impacts) if abs(i - p) <= 20
            ) / len(actual_impacts)
    ...
    def generate_performance_report(self, entity_type: str = "all") -> Dict:
        ...
        if (entity_type in ["campaigns", "all"]) and self.campaigns:
            ...
            if self.campaigns:
                ...
                sorted_by_conv = sorted(
                    self.campaigns, key=lambda c: c.get("conversions", 0), reverse=True
                )
                ...
                if campaigns_with_clicks:
                    ...
                    sorted_by_conv_rate = sorted(
                        campaigns_with_clicks, key=lambda c: c.get("conv_rate", 0), reverse=True
                    )
        ...
        if (entity_type in ["keywords", "all"]) and self.keywords:
            ...
            if self.keywords:
                # Sort by conversions
                sorted_by_conv = sorted(
                    self.keywords, key=lambda k: k.get("conversions", 0), reverse=True
                )
                ...
                if keywords_with_clicks:
                    ...
                    sorted_by_conv_rate = sorted(
                        keywords_with_clicks, key=lambda k: k.get("conv_rate", 0), reverse=True
                    )
    ...
    def _generate_insights(self) -> List[str]:
        ...
        if self.campaigns:
            ...
            if len(self.campaigns) > 1:
                ...
                if total_cost > 0:
                    ...
                    if max_cost_pct > 80:
                        insights.append(
                            f"One campaign accounts for {max_cost_pct:.1f}% of total spend"
                        )
        ...
        if self.keywords:
            ...
            if high_qs_keywords:
                ...
                insights.append(
                    f"{high_qs_pct:.1f}% of keywords have quality scores of 8 or higher"
                )
    ...
    def run_daily_optimization(self, days=None):
        ...
        try:
            ...
            logger.info(
                f"Auto-implementing recommendations with confidence >= {self.auto_implement_threshold}"
            )
            ...
            result["recommendations"]["failed"] = len(
                [r for r in recommendations if r.status == "failed"]
            )
            ...
            # Log summary
            logger.info(
                f"Daily optimization cycle completed: "
    ...
    def _apply_recommendation(self, rec: Recommendation) -> bool:
        ...
        try:
            logger.info(
                f"Applying recommendation: {rec.action_type} for {rec.entity_type} {rec.entity_id}"
            )
    ...
    def save_recommendations(self):
        ...
        try:
            ...
            logger.info(
                f"Updated recommendation history with {len(self.recommendation_history)} total recommendations"
            )
